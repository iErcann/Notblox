...
export class RandomizeSystem {
    update(entities) {
        for (const entity of entities) {
            if (!entity.getComponent(RandomizeComponent))
                continue;
            const sizeComponent = entity.getComponent(SizeComponent);
            if (sizeComponent) {
                if (Math.random() < 0.01) {
                    EventSystem.addEvent(new SizeEvent(entity.id, 1 + Math.random() * 4, 1 + Math.random() * 4, 1 + Math.random() * 4));
                }
            }
            ...
            if (rigidBodyComponent) {
                ...
                if (Math.random() < 0.05) {
                    rigidBodyComponent.body.applyImpulse(new Rapier.Vector3((Math.random() - 1 / 2) * 500, (Math.random() - 1 / 2) * 500, (Math.random() - 1 / 2) * 500), true);
                }
            }
        }
    }
}
//# sourceMappingURL=RandomizeSystem.js.map