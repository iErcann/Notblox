...
export class BoxColliderSystem {
    ...
    onComponentAdded(entity, event, world) {
        ...
        if (!sizeComponent) {
            ...
            console.warn('BoxColliderSystem : No SizeComponent found on entity. Using a default size of 1.0.');
        }
        const colliderDesc = Rapier.ColliderDesc.cuboid(sizeComponent.width, sizeComponent.height, sizeComponent.depth);
        if (rigidBodyComponent.body) {
            boxColliderComponent.collider = world.createCollider(colliderDesc, rigidBodyComponent.body);
        }
    }
}
//# sourceMappingURL=BoxColliderSystem.js.map