...
import { TrimeshColliderComponent, TrimeshCollidersComponent, } from '../../component/physics/TrimeshColliderComponent.js';
...
export class TrimeshColliderSystem {
    async update(entities, world) {
        const createEvents = EventSystem.getEventsWrapped(ComponentAddedEvent, TrimeshCollidersComponent);
    ...
    async onComponentAdded(event, world, entity) {
        ...
        if (!kinematicRigidBodyComponent.body) {
            console.error('TrimeshColliderSystem: KinematicRigidBodyComponent exist, but it hasnt been initialized, check the order of the systems.');
            return;
        }
        ...
        if (model) {
            model.scene.traverse((child) => {
                if (child instanceof Mesh) {
                    ...
                    physicsTrimeshCollidersComponent.colliders?.push(new TrimeshColliderComponent(event.entityId, collider));
                }
            });
            // Apply the position component if it exists, otherwise the trimesh collider will be at the origin (0, 0, 0)
            const positionComponent = entity.getComponent(PositionComponent);
            if (positionComponent) {
                kinematicRigidBodyComponent.body.setTranslation(new Rapier.Vector3(positionComponent.x, positionComponent.y, positionComponent.z), true);
            }
        }
    }
}
//# sourceMappingURL=TrimeshColliderSystem.js.map